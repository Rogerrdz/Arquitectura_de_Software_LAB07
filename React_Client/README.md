...
Para tiempo real (Parte 4 sobre esta base), agrega tambien:
...
## Tiempo real en el canvas (Parte 4)

La pantalla principal de Blueprints usa colaboracion en vivo con `STOMP`:

- `STOMP`: colaboracion en vivo contra el backend Spring (`/ws-blueprints`, `/app/draw`, `/topic/blueprints.{author}.{name}`).

Flujo recomendado para demo:

1. Cargar el mismo plano en 2 pestañas.
2. Seleccionar `STOMP` en ambas.
3. Dibujar con clicks y verificar sincronizacion en tiempo real.
